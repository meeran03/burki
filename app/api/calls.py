...
async def get_call_by_sid(call_sid: str, db: Session = Depends(get_db)):
    """
...
async def get_call_transcripts(
    call_id: int, speaker: Optional[str] = None, db: Session = Depends(get_db)
):
    """
...
async def get_call_sid_transcripts(
    call_sid: str, speaker: Optional[str] = None, db: Session = Depends(get_db)
):
    """
    ...
    # Get transcripts for the call
    transcripts = await CallService.get_call_transcripts(call_sid, speaker)
    return transcripts


@router.get("/{call_id}/recordings", response_model=List[RecordingResponse])
async def get_call_recordings(
    call_id: int, recording_type: Optional[str] = None, db: Session = Depends(get_db)
):
    """
...
async def get_call_sid_recordings(
    call_sid: str, recording_type: Optional[str] = None, db: Session = Depends(get_db)
):
    """
    ...
    # Get recordings for the call
    recordings = await CallService.get_call_recordings(call_sid, recording_type)
    return recordings
