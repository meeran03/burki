...
async def create_assistant(
    ...
    existing = await AssistantService.get_assistant_by_phone(db, assistant.phone_number)
    if existing:
        raise HTTPException(
    ...
    new_assistant = await AssistantService.create_assistant(db, assistant.dict())
...
async def update_assistant(
    ...
    existing = await AssistantService.get_assistant_by_id(db, assistant_id)
    if not existing:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Assistant with ID {assistant_id} not found"
    ...
    if assistant_update.phone_number and assistant_update.phone_number != existing.phone_number:
        phone_exists = await AssistantService.get_assistant_by_phone(db, assistant_update.phone_number)
        if phone_exists:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def delete_assistant(
    ...
    result = await AssistantService.delete_assistant(db, assistant_id)
